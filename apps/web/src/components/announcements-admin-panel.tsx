...
import { isAnnouncementDraft, PublishAnnouncementButton } from "@/components/publish-announcement-button";
...
type Props = {
  ...
  blocks: BlockDto[];
  units: UnitDto[];
  /** @deprecated Use canCreatePublished */
  canMutate?: boolean;
...
function AnnouncementCreateForm({
  ...
  const drawerTitle = canCreatePublished ? t("createTitle") : t("createDraftTitle");
  ...
  return (
    <FormDrawer triggerLabel={drawerTitle} title={drawerTitle} success={state.success}>
      <form action={action} className="grid gap-4">
        ...
        {canCreateDraft && !canCreatePublished ? (
          <p className="text-xs text-muted-foreground">{t("draftSubmitHint")}</p>
        ) : null}
        {state.error ? (
        ...
        <div className="flex flex-wrap gap-2">
          {canCreateDraft ? (
            <Button type="submit" name="saveMode" value="draft" disabled={pending} variant="outline" className="flex-1 sm:flex-none">
              {t("saveDraft")}
            </Button>
          ) : null}
          {canCreatePublished ? (
            <Button type="submit" name="saveMode" value="publish" disabled={pending} className="flex-1 sm:flex-none">
              {t("publish")}
            </Button>
          ) : null}
        </div>
...
export function AnnouncementsAdminPanel({
  ...
  units,
  canMutate = true,
  canCreatePublished = canMutate,
  canCreateDraft = false,
  canPublish = canMutate,
  listBasePath,
}: Props) {
  const t = useTranslations("announcements");
  const paginationBasePath = listBasePath ?? `/admin/properties/${propertyId}/announcements`;
  const showCreateForm = canCreatePublished || canCreateDraft;
      ...
      <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
        ...
        {showCreateForm ? (
          <AnnouncementCreateForm
            ...
            blocks={blocks}
            units={units}
            canCreatePublished={canCreatePublished}
      ...
      <CardContent className="space-y-4">
        {items.length === 0 ? (
          ...
          <ul className="space-y-4">
            {items.map((item) => {
              const isDraft = isAnnouncementDraft(item);
              const publishStatus = isDraft
                ? null
                : getAnnouncementPublishStatus(new Date(item.publishStartAt), new Date(item.publishEndAt));
              return (
              <li key={item.id} className="rounded-lg border p-4">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  {item.isPinned ? <Badge variant="secondary">{t("pinned")}</Badge> : null}
                  {isDraft ? <Badge variant="outline">{t("statusDraft")}</Badge> : null}
