...
import { StaffMovementType } from "@siteyonetim/db";
...
export async function matchBankLineAsStaffMovementAction(
  locale: string,
  propertyId: string,
  _prev: BankingActionState,
  formData: FormData,
  ...
  try {
    await getBankingService().matchLineAsStaffMovement({
      organizationId: session.user.organizationId,
      propertyId,
      actorUserId: session.user.id,
      lineId: String(formData.get("lineId") ?? ""),
      staffProfileId: String(formData.get("staffProfileId") ?? ""),
      movementType: String(formData.get("movementType") ?? StaffMovementType.PAYMENT) as StaffMovementType,
      categoryId: String(formData.get("categoryId") ?? ""),
      description: String(formData.get("description") ?? "") || null,
    });
    revalidateBanking(locale, propertyId);
    return { success: true, matched: 1 };
  } catch (error) {
    if (error instanceof Error) {
      return { error: error.message };
    }
