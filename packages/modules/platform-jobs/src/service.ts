...
import type { BankingServiceContract } from "@siteyonetim/finance-banking";
...
export class JobService implements JobServiceContract {
  ...
  private readonly banking: BankingServiceContract;
  ...
  constructor(
    ...
    banking: BankingServiceContract = createBankingService(),
  ) {
    this.dues = dues;
  ...
  async runBankStatementSync(input: RunBankStatementSyncInput): Promise<RunBankStatementSyncResult> {
    return this.banking.syncRestPollProfiles(input.actorUserId);
  }
}
...
export function createJobService(
  ...
  banking?: BankingServiceContract,
): JobService {
  return new JobService(dues, notifications, auditorReports, banking);
}
